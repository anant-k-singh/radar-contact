...
| Which SID figures are printed | **Ceilings always; floors only at a turn, the last fix, or the first after the runway.** A SID's published floors step up at every fix, which down LSGG's MEDAM 1A prints five ascending numbers along a line that never turns. A floor describes what the departure does anyway, so it is worth reading where the track changes or where it leaves; a ceiling is the restriction holding it under an arrival (§4.7) and is published mid-run *because* that is where the crossing is. Thinning it would delete the one SID figure the controller cannot infer — ZZZZ's MORVA and TELMU are exactly that. The straight-through threshold is `STRAIGHT_OUT_DEG`, borrowed rather than invented |
