...
- **A field may state its terrain, as bands of minimum safe altitude, and it is scenery as well.**
  ...
  - **The figures are not drawn on the scope at all; the MSA is read at the pointer.** The bands
    carry no printed label. `MSA @ pointer` in the stats block answers the same question on demand,
    for the exact spot being asked about rather than for a band as a whole. See the decisions
    register.
...
### 4.5a Rejoining an arrival (`R`)

Pulling one of two converging arrivals off its route to deconflict them, then putting it back on and
letting the extra track miles do the sequencing, is the ordinary radar technique. Without a way back
onto the route only the first half of it existed, and every vectored aircraft had to be flown to the
ILS by hand. `R` is the second half, and it means one thing — *resume the arrival* — with three
readings decided by the aircraft's state, exactly as `H` has three.

**On the route, with an altitude or a speed assigned**, `R` clears those overrides and the published
profile takes its axes back. Nothing lateral changes. This is the `W`/`S`/`Q`/`E` takeover of §4.5
...
- **It is the question the scope already answers.** "Which leg?" is read off the heading vector drawn
  across the route, rather than computed from geometry the display cannot show. Every other selection
  rule leaves the player guessing.
...
are heard when they happen. The route is still remembered afterwards, so a second `R` re-casts the ray
from wherever the aircraft now is.
...
distance to the joining fix plus the route's own distance from there, so it arrives at the leg
already on the profile and the capture is lateral only. Any axis the controller had taken is given
back at the same moment, because that is what *resume* means.

Giving it the joining fix's level as a plain assignment instead was tried first and is wrong: the
aircraft dives at the full kinematic rate and levels off early. Measured on ZZZZ's RIMOL 1A, a
rejoining arrival descended at **1400 fpm** past an untouched one alongside it doing **650** — the
dive-and-drive §4.5 exists to avoid, and visible on the scope as two aircraft on the same arrival
behaving differently.

Two cases the hold path never produced. **Rejoining from below**: an aircraft under its profile is
not climbed back up to it — it holds its level and lets the descending profile come down to meet it,
which is the same crossing test read from the other side, and it is why `StarNav.rejoining` is signed
rather than boolean. And **from above the target leads the profile by `ALT_CAPTURE_FT`** rather than
sitting on it: the vertical rate tapers to 15 % inside that band (§4.3), so an aircraft creeping onto
a profile that is itself descending at 700 fpm converges to a hundred feet above it and stays there,
never crossing — the capture never fires and the published crossings are never made good. Aiming
through the profile means the descent actually reaches it, and the crossing is still the capture.
...
radials, not to STARs. The intercept is chosen here as a gameplay device, because it puts setting up
the join in the player's hands and because a direct-to falls out of it anyway: aim at a fix and the
ray crosses there. §14 records the departure from the phraseology rather than laundering it.
...
aircraft picks the descent back up where the hold interrupted it. A hold is not a rejoin at all —
the aircraft never left the route — which is why it works with no intercept and no geometry, unlike
`R` (§4.5a).
...
**Rejoining is flown, not jumped.** The profile is normally written straight onto the aircraft,
...
vertical rate in the millions of fpm). So off the profile it flies *towards* it on ordinary kinematic
rates, and `starOwnsVertical` reports false for those ticks so kinematics integrate the vertical.
**The capture test is simply "no longer on the side it started"**, so the crossing is the capture and
no tolerance window is needed — which matters, because any window wide enough to be safe is also wide
enough to be a visible step, and one narrow enough to be invisible breaks ordinary arrivals where the
profile levels off at a platform faster than the aeroplane can follow.

`StarNav.rejoining` is therefore **signed** — 1 above, -1 below, 0 on it — rather than a boolean.
A pattern can be flown *below* its published crossing just as legally as above it, since the holding
level is given back on exit either way, and the boolean version read that aircraft as "on the
profile" and wrote the chart straight on: an instant climb of thousands of feet. From below the
aircraft **holds its level** and lets the descending profile come down to meet it; an arrival is
never climbed back up to a profile it is under. That is the same case `R` produces from a vector
(§4.5a), which is why one mechanism serves both.
...
| `R` | Resume the arrival: hand the published profile back, or arm a rejoin off the route (§4.5a) |
| `Tab` | Cycle selection |
| `Space` | Pause / resume |
...
- **Data block**, two lines, offset from the blip:
  ```
  KLM133 ARDIS    ← callsign + state tag (next fix, →fix while rejoining, ILS/LOC/G/S, G/A or TWR)
...
| Where a band's figure is placed | **Nowhere — it is read at the pointer instead** (2026-09-05, LSGG). Superseded by the row below. The original decision follows. **Outside the boundary, on a leader line, one per band.** Labelling from inside does not work here and the bands' size is not why: the compass rose prints its bearings 24 px inside the boundary and the gates own the edge on their radials, so the eastern third of the airspace — exactly where the terrain is — is already spoken for, and an interior `MSA 4000` lands on the `090` tick while a coastal one collides with the fix labels being read. The chart is the working layer and terrain is context, so context moves to the margin. A leader crossing the boundary once and ending on bare background cannot be mistaken for a crossing restriction, which is also why the `MSA` prefix is no longer needed. One per band, not per ring: the figure names a step in the ramp, and the fill already says which patch belongs to which band |
...
| Question | Decision (2026-09-05, LSGG in play) |
| --- | --- |
| How a controller reads the terrain | **At the pointer, not off the map.** The per-band leader lines worked at VABB, where four bands cover the eastern third of the scope. LSGG has fourteen bands over most of the airspace, and fourteen figures stacked down the margin said which levels exist without saying where any of them is — the reader still had to match a shade against a legend. `MSA @ pointer` in the stats block answers for the exact spot instead, so the labels came off entirely and `drawTerrainCallouts` with them. It is blank over an aircraft or its data block: a figure that changes as the pointer crosses a target competes with the one readout on the scope that already means something specific |
| How many shades the ramp has | **As many as the field has bands, stretched between two theme colours.** It was a fixed array of four with the top entry repeated once it ran out — which was invisible at VABB, whose four bands fit exactly, and painted **eleven of LSGG's fourteen the same shade**: everything above 7000 was one flat mass. `terrainRamp(bands)` interpolates instead. Per field rather than one fixed altitude→colour scale, so a field's terrain reads against itself first; the cost is that a shade cannot be read as an altitude on its own, which is what the pointer readout is for. Interpolated in sRGB, not Lab — these are small dark patches over a narrow range, and the difference would be less visible than the machinery |
| Why the ramp is shaped rather than linear | **A fixed sRGB step is worth less near black.** Linear interpolation gave LSGG's darkest three bands — 4000, 5000, 6000, which are also its three largest by area — steps of 3.8–4.4 luminance, not readable at that level, while the bright end had steps to spare. A gamma of 0.90 front-loads the range: the first three steps become 5.5/5.4/4.7 and the crowded upper bands give up about a tenth of a unit each. Both ends of the range are pinned by something real and neither is free: the top by `starPath`, since VABB draws arrival routes directly over its two brightest bands (0.0 and 0.1 NM); the bottom by the background, since the lowest band has no darker neighbour and the background *is* its contrast. Taking the low end down to 1.9× the background to buy wider steps was tried and reverted — the 4000 band stopped being visible, which is the same complaint one band lower. 2.2× is where it reads, so the range is fixed and the shaping is the only free variable |
| How finely a field may band its terrain | **Bounded by contrast, at about fourteen bands.** The ramp's top has to stay under `starPath` or the STAR lines stop reading against the ground they cross, and that ceiling fixes the usable range; LSGG's fourteen bands divide it into steps of ~3.8 luminance, near the floor for irregular patches on a dark ground. Adjacent bands are distinguishable, an isolated patch is not — which is the honest limit and the reason the exact figure is read at the pointer. Pinned in `tests/terrain.test.ts` so a fifteenth band or a wider ramp is a decision rather than a slide back into one flat mass |
| Which fill rule the readout uses | **Nonzero winding, matching `ctx.fill()`.** The lookup has to answer the question the shading answered or the two disagree under the cursor. A band's holes are wound the other way on purpose so the fill subtracts them, and even-odd cannot tell a hole from an overlap — pinned in `tests/terrain.test.ts` |
| LSGG's MVA against its platform | **5000, two steps below the 7000 every arrival is delivered to.** One step was not enough: an aircraft that misses the localizer intercept has to be taken down again to re-establish, and 6000 gave that a single 1000 ft try. At 5000 the glideslope is reachable from 11.3 NM rather than 14.5, which is the room to re-establish inside the base turn instead of going round. The ground allows it where it is used — the extended centreline is clear of shaded terrain from the threshold to 26 NM at ±3 NM either side, the nearest 4000 band being the Salève 5 NM left of course — and 5000 is still 3589 ft above the field. The original reasoning follows. **One step below the 7000 every arrival is delivered to.** The published MSA within 10 NM is 7000 and so are BIVLO, PITOM, INDIS and the two holding bases, but an MVA equal to the platform leaves nothing to descend into: an aircraft vectored off its STAR is stuck at the level it left. The terrain shading is not derived from `mvaFt` and does not move with it, so the two disagree over 8 % of the airspace exactly as at VABB, and for the same documented reason |
| Whether VALBU stays on the southern STARs | **Dropped.** It sits 0.01 NM off the direct GOLEB–SUVEL line and turns the track by 0.2°, so it was a published level on a straight leg rather than a shape in the route — and the observed GOLEB 16,000 → BIVLO 11,000 gradient of 321 ft/NM already passes through where it was. Removing it changes the flown path by nothing and the profile by one interpolated step |
| The downwind speed at LSGG | **210 from GG507/GG525 to the platform.** The charts publish 220 at the base turn, which GG514 and GG512 carried, and slowing the two fixes before them to 210 left every southern and northern route accelerating 210 → 220 → 210. No chart codes that. Both streams now decelerate once, on the downwind, and hold it — the speed is monotonic from gate to platform on all nine routes |
| Modelling "turn when passing 7000, but not before PAS" | **A new opt-in `SidFixSpec.turnAtOrAboveFt`, read by `stepDeparture`.** Every LSGG RWY 22 sheet carries this and nothing in the model could express it, so the turn off PAS was purely lateral and the A332 — the only type in the fleet climbing at 2000 fpm — rounded it low and crossed the Jura **490 ft under its 7000 MSA**. Deliberately not `minAltitudeFt`: that is validator-only and describes what a departure does anyway, and gating VABB's floors on it would have departures orbiting fixes waiting for levels they had long since climbed through. The gate blocks *sequencing*, not the climb, so it sits after `ceilingAtFt` and before the anticipation check — the aircraft keeps climbing while held |
| What a held departure flies while waiting for the gate | **The leg it arrived on, extended.** The chart says "not before PAS", and what follows a fix you may not turn at is the inbound track continued — which at LSGG is runway heading out over the Rhône valley, the one direction known to be clear. Steering at the fix instead is what a naive gate does and it is not merely cosmetic: the aircraft reaches the fix, keeps aiming at it, and flies a complete orbit — measured at 257°, 319°, 14°, 91°, 167°, all within 4.5 NM of PAS and pointed back at the field. Both halves are pinned in `tests/scenario.test.ts`, and each fails on its own mutation |
| Which SID figures are printed | **None, at any field** (2026-09-05). Superseded the row below, which is kept because its reasoning is why the hover exists. A SID is drawn as rings and a track; every level is read by hovering it (`sidHover.ts`), which prints all of them. The thinning rule below was a good rule and the problem was that it was a rule: it left a departure's levels partly readable and partly not, with the split invisible to the reader — LSGG printed ten floors and two ceilings over ground already carrying nine STARs and fourteen terrain bands. Removing it deletes `STRAIGHT_OUT_DEG` from the render layer entirely. The original decision follows. **Ceilings always; floors only at a turn, the last fix, or the first after the runway.**
| A published entry fix inside the boundary | **Pushed out to the edge along its own next leg** (`extendToRange`, the mirror of `clipToRange`). An arrival spawns at its gate, so a gate inside the airspace puts one on the scope already in controlled airspace with no run in from the edge — the controller sees it appear rather than arrive. Three of LSGG's nine sit 8 to 15 NM inside a 55 NM boundary. Extending backwards up the leg the aircraft would have been flying anyway adds distance without inventing a turn, and the published fix stays on the route as its first waypoint; the boundary point is the invention, so it takes an `RC` name (`RCBE`, `RCBA`, `RCLU`) as VABB's three do |
| KONIL 1R | **Dropped, not modelled.** It runs parallel to the right downwind 3 NM off it and then crosses the base leg, and the separation that makes it work in the real sector is tactical control this model does not give a departure. Its fixes stay in `fixes.ts`, which is the transcription record rather than the route list. Superseded in part by the rows below: the traffic it was to have carried now goes on SOSAL 1J, not on the SOSAL 1L named here |
| Reading a SID's levels the chart thinned away | **Hover the track.** The thinning is right — a departure's floors otherwise print a column of ascending numbers along a line that never turns, five of them down MEDAM 1A — but it is lossy, and the levels are in `scenario.sids` the whole time with no way for the player to ask. Hovering prints every fix *name*, which the chart layer never draws at all, and every published level including the dropped ones. Drawn per frame in `sidHover.ts` rather than into `mapLayer`: that layer is an offscreen canvas keyed on field and viewport, so painting a hover into it would rebuild the whole chart on every pointer move. The cost is ~25 point-to-segment tests and under ten labels against the per-aircraft blocks already drawn at 20 fps. Both layers go through `sidFixLabels`, so "would the chart have printed this" is asked once — two copies of that rule drift into a doubled label. Suppressed over an aircraft and in replay, for the reasons `MSA @ pointer` is |
| What Q/E step from on a STAR | **The speed shown in the data block — the next fix's — not the autopilot's target** (2026-09-06). While an aircraft decelerates between two fixes with different speeds, `targetIasKts` slides continuously between them while the block shows the constraint ahead. Stepping from the target made the key unpredictable: on ZZZZ's KOVAL 1A, passing 229 kt towards BOXAR's 200, the block read 200 and one press of E gave **240** where the player expected 210. `adjustSpeed` now takes the same expression the sidebar renders. A hold is excluded — a holding aircraft is at `HOLD_SPEED_KTS`, not the route's speed, which the existing hold test caught |
| Why LSGG's southern SIDs turn 1000 ft lower | **The gate is per fix, not per field** (2026-09-06). "When passing 7000, but not before PAS" is set by the Jura north-west of the field; south of PAS the turn sector tops out at MSA 6000, so MEDAM 1A and BEVEN 1A carry 6000 instead. Flown, the terrain margin *improves* — 2537 to 2598 — because turning sooner takes the aircraft away from the high ground earlier, and arrival separation is unchanged. DIPIR 1A keeps 7000 because it turns over the Jura itself; DEPUL 1A keeps it because at 235° against a 226° runway it barely turns, and the gate buys it 3 ft |
| Whether every fix can be held at | **No — a fix with no published level is not a holding fix** (2026-09-06). Follows from a STAR fix being allowed to omit its altitude: `enterHold` anchors the pattern on the level at the fix, so at a bare one it would take whatever height the aircraft happened to be passing — a different level every time, and not one the controller chose or can predict. `toggleHold` refuses with "not a holding fix" rather than inventing one. Leaving a hold is unaffected: the refusal is on entry only, so an aircraft already in a pattern can always be told to leave it |
| LSGG's southern handovers, and the ceiling | **BANKO 3R and BELUS 3R hand over at 21,000, which lifts `CEILING_FT` to 21,000** (2026-09-06). The field's rule is that the ceiling is set *by* the highest handover, never chosen — the controller has to be able to hold an arrival at the level it arrives on — so raising the two southern gates raises the ceiling with them. LUSAR's published FL200 now sets a floor under that number rather than being the number itself. Every descent gradient stays under 331 ft/NM, about 3° |
| Whether a STAR fix must publish an altitude | **Only the one the route ends at.** The rule was "both an altitude and a speed, at every fix", which forces a level onto a fix that is merely *on* a descent — LSGG's GG502 sits on CBY's 3° gradient into PITOM, so its 14,000 restated what the two ends already said. `compileStar` drops the gaps and `starProfileAt` interpolates across them, so the flown profile is unchanged (GG502 is crossed at 14,606 on the interpolated line). The last fix stays required: it is the level the handover happens at and the glideslope check reads it. The first needs no check — the gate waypoint is synthesised from `entryAltitudeFt`, which is required separately |
| Where a departure levels off with nothing left to cross | **30,000 on every field, a cruise level rather than a margin over the ceiling.** It was `ceilingFt + 1000`, which made an arbitrary number look like a restriction and produced the conflict it was meant to prevent: LSGG's MEDAM 1A sat at 21,000 against a KINES 2R arrival entering at 20,000, exactly 1000 ft apart and nearly head-on at the boundary. A departure with every restriction behind it is not being held by anyone — it is climbing away, and the model only has to stop it somewhere. Departures now leave the airspace **still climbing**, at 19,000–28,000 depending on type and route, which is what a real one does |
| What altitude does to the climb rate | **Linear decay from 10,000 to 30,000, down to 0.35 of book** (`CLIMB_DECAY_*`). The APD figure is a low-level rate; holding it to 30,000 would have a jet climbing as steeply at the top as off the runway, which none does — thrust falls with density while the weight does not. The floor is at 10,000 deliberately: **nothing below it changes at all**, so every observed level on every SID at all three fields is exactly what it was, and the decay only shapes the part of the climb that used to be truncated by the old top. Measured on pressure altitude rather than AGL, the opposite datum from `INITIAL_CLIMB_REDUCTION_FPM`, because it is the air that thins and not the height above the field |
| A SID's exit fix and the top of climb | **Decoupled.** `compileSid` used to default the last fix's floor to `topFt`, on the reasoning that a chart labels the top of climb there. That was true while the top was a margin over the ceiling and false once it became a cruise level: an exit fix inside the boundary is nowhere near 30,000, so the default authored a level no departure makes and three tests asserted it was reached. A field wanting a figure at its exit publishes one; the conformance suite now asks the question that still means something — did the departure get **above the assignable ceiling** before it left |
| Which north-east SID LSGG flies | **SOSAL 1J (chart 22-11), replacing SOSAL 1L.** 1L was authored first and climbed a departure straight through the arrival stream — flown, an A320 met an AKITO 3R arrival at 8955 ft with **0 ft between them**, which is the one mistake the field exists to teach. 1J is also what the live picture shows leaving north-east. It is the only SID here that turns at the field rather than at PAS, and after GG603 it runs **direct to SOSAL**, 38.5 NM up the middle of the lake |
| How SOSAL 1J separates without a restriction | **On distance, not on a ceiling.** The direct leg crosses the left downwind dead-on at 21 NM — 0.02 NM lateral, so there is no lateral separation to be had — but 21 NM of unrestricted climb puts the slowest type in the fleet **2762 ft** above the arrival there and the rest 5089 to 7443 above. This matters because Geneva publishes no "at or below" anywhere: a field that cannot hold a departure down has to out-climb the conflict instead, and the straight leg is what buys the distance to do it. Authoring a 7000 ceiling at GG607 was the alternative and was not taken — it would have been our number, not the chart's |
| Where SOSAL 1J turns | **GG601, at 1900 ft, over the field — not PAS at 7000.** Every other RWY 22 SID turns at PAS and 1J's chart says "when passing 1900, but not before GG601". Authoring it with the others' gate does not merely differ from the chart, it does not fly: PAS is 6.4 NM off the departure end and nothing reaches 7000 that soon, so the gate released **3.0 NM past** the fix with GG603 9.6 NM behind and to the right, and the aircraft turned 155° away from the field to come back for it — 27.6 NM of track to reach a fix 13.9 NM along the route, arriving at 14,400 ft where the live picture shows 6000. The gate is made *before* the fix here, which is the point: what holds the turn is the fix, not the level |
| Where LSGG's downwind arrivals stop | **At the base turn, not at SAPRE.** Both base turns are charted "allowed with ATC clearance only" — the same statement as VABB's closing `VM` vector — so coding the six downwind routes through to SAPRE had them turning themselves onto final, which is the one part of the job the field exists to hand over. They now end at GG514 and GG512, on downwind heading, and wait to be vectored. The three north-east routes keep SAPRE because they have no downwind and no base turn: they are straight-in, and the IAF is where they genuinely end |
| GG512's level at the base turn | **7000, down from 8000.** The two downwinds are symmetric — GG514 is 18.6 NM out and GG512 18.7 — so 8000 against GG514's 7000 was an asymmetry nothing had to answer for while SAPRE followed and pulled both to 7000 a mile later. A route that *ends* at GG512 has to be handed over somewhere the approach can be given, and 8000 there is **628 ft above the glideslope**, which is `validateScenario`'s own finding once the route stopped there |
| The two unnamed fixes on SOSAL 1J's direct leg | **Ours, for the validator rather than the aircraft** — hence `RCGA`/`RCGB`, the `RC` prefix VABB established for a fix that is not off the chart. `checkSidStarClearance` compares a SID's *published band* against the arrival, and a 38.5 NM leg with no floors declares 0 ft, which reads as a 7583 ft bust at a crossing the traffic clears by thousands. Two floors on the straight line narrow the band to what is flown and the finding goes away. They sit on the line `alongLeg` draws between GG603 and SOSAL, so they move the flown path by nothing. The A332 makes 8466 at `RCGA`'s 9500 — a floor the heaviest type does not quite make, kept because it is still 2762 ft clear and because the chart's own note says this SID is not for the noise classes the heavies fall in |
...
6. **~~Rejoining a STAR~~** Resolved 2026-09-06: `R` gives the route back (§4.5a), by arming an
