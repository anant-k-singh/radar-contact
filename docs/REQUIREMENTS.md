...
| Why the ramp is shaped rather than linear | **A fixed sRGB step is worth less near black.** Linear interpolation gave LSGG's darkest three bands — 4000, 5000, 6000, which are also its three largest by area — steps of 3.8–4.4 luminance, not readable at that level, while the bright end had steps to spare. A gamma of 0.90 front-loads the range: the first three steps become 5.5/5.4/4.7 and the crowded upper bands give up about a tenth of a unit each. The range was also widened **downward, not upward** — the top is fixed by `starPath` and that is a live constraint, since VABB draws arrival routes directly over its two brightest bands (0.0 and 0.1 NM), whereas the bottom had unused headroom against the background |
