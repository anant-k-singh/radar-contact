...
| KONIL 1R | **Dropped, not modelled.** It runs parallel to the right downwind 3 NM off it and then crosses the base leg, and the separation that makes it work in the real sector is tactical control this model does not give a departure. SOSAL 1L leaves in the same direction and carries the traffic; KONIL is also the rarest departure on the live picture, so the field loses almost nothing. Its fixes stay in `fixes.ts`, which is the transcription record rather than the route list |
